      ...
      import("leaflet").then((L) => {
        ...
        const createCustomIcon = (severity: string, anomalyType: string) => {
          ...
          return L.divIcon({
            html: `
              <div style="
                background-color: ${colors[severity as keyof typeof colors] || "#6b7280"}; 
                width: 30px; 
                height: 30px; 
                border-radius: 50%; 
                border: 3px solid white; 
              ...
              <div style="padding: 12px; min-width: 200px;">
                ...
                ${
                  alert.snapshotUrl || alert.snapshot_url
                    ? `
                  <a href="${alert.snapshotUrl || alert.snapshot_url}" target="_blank"
                      style="display: inline-block; margin-top: 8px; padding: 4px 8px;
                             background: #3b82f6; color: white; text-decoration: none;
                             border-radius: 4px; font-size: 12px;">
                    📸 View Snapshot
    ...
    <Card className="bg-slate-900 border-slate-700">
      <CardHeader>
        <CardTitle className="text-blue-400 flex items-center gap-2">
          ...
          <span className="text-sm font-normal text-slate-400">({alerts.length} alerts)</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div ref={mapRef} className="w-full h-96 rounded-lg border border-slate-700" style={{ minHeight: "400px" }} />
        <div className="flex items-center gap-6 mt-4 text-sm">
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 bg-green-500 rounded-full border-2 border-white shadow"></div>
            <span>Low Severity</span>
